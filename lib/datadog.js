...
var flush_stats = function datadog_post_stats(ts, metrics) {
   ...
   var key;
   if (debug) {
      util.log('DEBUG:['+ts+']:datadog-backend: raw metrics: ' + JSON.stringify(metrics));
   }
   // Send counters
   for (key in counters) {
      if (skipInternalMetrics && key.match(internalStatsdRe) != null) {
        if (debug) {
           util.log('DEBUG:['+ts+']:datadog-backend: skipping internal counter metrics: ' + JSON.stringify(counters));
        }
        continue;
      }
      value = counters[key];
   ...
   for (key in gauges) {
      if (skipInternalMetrics && key.match(internalStatsdRe) != null) {
        if (debug) {
           util.log('DEBUG:['+ts+']:datadog-backend: skipping internal guage metrics: ' + JSON.stringify(guages));
        }
        continue;
      }
      value = gauges[key];
   ...
   for (key in timers) {
      if (skipInternalMetrics && key.match(internalStatsdRe) != null) {
        if (debug) {
           util.log('DEBUG:['+ts+']:datadog-backend: skipping internal timer metrics: ' + JSON.stringify(timers));
        }
        continue;
      }
...
var get_tagged_metric_tags = function datadog_get_tagged_metric_tags(key) {
    ...
    if(logAll){
      util.log('DEBUG:datadog-backend: Metric tag string: ' + metricTagString);
