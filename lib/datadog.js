...
var flush_stats = function datadog_post_stats(ts, metrics) {
   ...
   for (key in counters) {
      if (skipInternalMetrics && key.match(internalStatsdRe) != null) {
        if (debug) {
           util.log('DEBUG:['+ts+']:datadog-backend: skipping internal counter metrics: ' + JSON.stringify(key));
        }
        continue;
      }
      value = counters[key];
   ...
   for (key in gauges) {
      if (skipInternalMetrics && key.match(internalStatsdRe) != null) {
        if (debug) {
           util.log('DEBUG:['+ts+']:datadog-backend: skipping internal guage metrics: ' + JSON.stringify(key));
        }
        continue;
      }
      value = gauges[key];
   ...
   for (key in timers) {
      if (skipInternalMetrics && key.match(internalStatsdRe) != null) {
        if (debug) {
           util.log('DEBUG:['+ts+']:datadog-backend: skipping internal timer metrics: ' + JSON.stringify(key));
        }
        continue;
      }
