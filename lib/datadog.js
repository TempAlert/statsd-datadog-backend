...
Datadog.prototype._post = function(controller, message) {
    ...
    request.post({
        ...
        if (error) {
            util.log('Skipping, cannot send data to Datadog: ' + error.message);
        }
        client.pending_requests -= 1;
    });
};
...
exports.init = function datadog_init(startup_time, config, events, log) {
   ...
   util.log('Leading datadog backend...')
   hostname = config.hostname;
   if (config.datadog) {
        datadogApiKey = config.datadog.datadogApiKey;
        datadogApiHost = config.datadog.datadogApiHost;
        datadogPrefix = config.datadog.datadogPrefix;
        datadogTags = config.datadog.datadogTags;
   ...
   if(!datadogApiKey) {
    util.log('Invalid configuration for DataDog backend. No API Key.', 'LOG_CRIT');
    return false;
   }
