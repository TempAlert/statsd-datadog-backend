...
Datadog.prototype._post = function(controller, message) {
    ...
    request.post({
        ...
        if (error) {
            util.log('ERROR:datadog-backend: Skipping, cannot send data to Datadog: ' + error.message);
        }
        client.pending_requests -= 1;
    });
};
...
var post_stats = function datadog_post_stats(payload) {
   try {
       ...
       if (debug) {
          util.log('WARN:datadog-backend: Skipping payload. It was either null or had no data');
       }
   } catch(e){
      if (debug) {
         util.log('ERROR:datadog-backend: Skipping, cannot send data to Datadog: ' + e);
      }
...
var flush_stats = function datadog_post_stats(ts, metrics) {
   ...
   var key;
   if (debug) {
      util.log('DEBUG:datadog-backend:['+ts+']: raw metrics: ' + JSON.stringify(metrics));
   }
   // Send counters
   for (key in counters) {
      if (skipInternalMetrics && key.match(internalStatsdRe) != null) {
        if (debug) {
           util.log('DEBUG:datadog-backend:['+ts+']: skipping internal counter metrics: ' + JSON.stringify(key));
        }
        continue;
      }
      value = counters[key];
   ...
   for (key in gauges) {
      if (skipInternalMetrics && key.match(internalStatsdRe) != null) {
        if (debug) {
           util.log('DEBUG:datadog-backend:['+ts+']: skipping internal guage metrics: ' + JSON.stringify(key));
        }
        continue;
      }
      value = gauges[key];
   ...
   for (key in timers) {
      if (skipInternalMetrics && key.match(internalStatsdRe) != null) {
        if (debug) {
           util.log('DEBUG:datadog-backend:['+ts+']: skipping internal timer metrics: ' + JSON.stringify(key));
        }
        continue;
      }
...
var get_host_from_metric = function datadog_get_host_from_metric(key) {
    ...
    if (hashLocation > -1){
        ...
        for (var i = 0, len = keyTagPairs.length; i < len; i++) {
            var ktp = keyTagPairs[i].split('=');
            if(ktp[0].toLowerCase() == "host"){
                if(logAll){
                  util.log('DEBUG:datadog-backend: Found host tag: ' + ktp[1]);
                }
                return ktp[1];
            }
        }
    ...
    if(logAll){
      util.log('DEBUG:datadog-backend: No host tag found. Using existing: ' + hostname);
    }
...
var get_tagged_metric_tags = function datadog_get_tagged_metric_tags(key) {
    ...
    if (hashLocation  == -1){
        if(logAll){
          util.log('DEBUG:datadog-backend: Metric was not tagged. Returning datadogTags: ' + datadogTags);
        }
        return datadogTags;
    }
    ...
    var metricTags = metricTagString.replace(/=/g,':').split(',');
    // append the datadog tags if they are set
    if (datadogTags){
        if(logAll){
          util.log('DEBUG:datadog-backend: DatadogTags set. Pushing them into tags. datadogtags: ' + datadogTags);
        }
        Array.prototype.push.apply(metricTags, datadogTags);
    }
    if(logAll){
      util.log('DEBUG:datadog-backend: Metric tag array: ' + JSON.stringify(metricTags));
    }
    return metricTags;
}
...
var get_tagged_metric_key = function datadog_get_tagged_metric_key(key) {
    ...
    if (hashLocation  == -1){
        if(logAll){
          util.log('DEBUG:datadog-backend: Metric was not tagged: ' + key);
        }
        return key;
    }
    var keyWithNoTags =  key.substring(0, hashLocation);
    if(logAll){
      util.log('DEBUG:datadog-backend: Metric was tagged. Key with no tags is: ' + keyWithNoTags + ' original key: ' + key);
    }
...
exports.init = function datadog_init(startup_time, config, events, log) {
   ...
   util.log('INFO:datadog-backend: Loading datadog backend...');
   ...
   if(!datadogApiKey) {
    util.log('ERROR:datadog-backend: Invalid configuration for DataDog backend. No API Key.');
    return false;
   }
   events.on('flush', flush_stats);
   events.on('status', backend_status);

   util.log('INFO:datadog-backend: Loaded datadog backend.');
   return true;
};
